...
**Color Notes:** Vibrant rose + engagement blue. (Targeted for campus/student vibe).

### Typography

- **Heading Font:** Lexend
- **Body Font:** Source Sans 3
- **Mood:** youthful, readable, accessible, clean, energetic
...
- ❌ Forcing linear unskippable onboarding tours (Always provide Skip/Back buttons)
- ❌ Empty or missing alt attributes on images (Always provide descriptive alt text)
- ❌ Visual-only error indication (Use `aria-live` or `role=alert` for errors)
- ❌ Untyped navigation params (Use typed navigation props: `navigation.navigate<RootStackParamList>('Home', { id })`)
- ❌ Inline render functions in lists (Always optimize `renderItem` using `React.memo`)
...
- [ ] Error states include `aria-live` or `role='alert'`
- [ ] React Native Navigation uses typed params
- [ ] React Native FlatLists use memoized `renderItem` components
