    ...
    if (this.state.hasError) {
      return (
        <View className="flex-1 items-center justify-center bg-[#fff7f2] px-6">
          <View className="w-full max-w-sm rounded-2xl bg-white p-6" style={{ elevation: 3 }}>
            <Text className="text-2xl font-bold text-slate-900">{this.props.moduleName}</Text>
            <Text className="mt-2 text-base text-slate-600">
              This module crashed but other tabs remain usable.
            </Text>
            {this.state.message ? (
              <Text className="mt-2 text-sm text-rose-700">{this.state.message}</Text>
            ) : null}
            <Pressable
              className="mt-5 rounded-xl bg-rose-600 px-4 py-3"
